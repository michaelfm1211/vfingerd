...
static bool
isBooleanValue(const char* key, const char* value)
{
    if (strcmp(value, "true") && strcmp(value, "false")) {
        fprintf(stderr,
            "%s: Invalid value for key '%s' on line %d\n",
            debug.path,
            key,
            debug.linenum);
        return false;
...
static void
list_add_node(struct config_ent** list, struct config_ent** tail)
{
...
// contents.
static bool
list_node_plan_file(struct config_ent* node,
    const char* path,
    bool suppress_errors)
{
    ...
    if (plan == NULL) {
        if (!suppress_errors) {
            fprintf(stderr,
                "%s: Error opening file on line %d\n",
                debug.path,
                debug.linenum);
            perror(path);
        }
...
static bool
list_node_use_passwd(const char* name, struct config_ent* node)
{
...
static bool
list_node_set(struct config_ent* node, char* key, char* value)
{
    if (node == NULL) {
        fprintf(stderr,
...
static char*
heredoc(FILE* file, const char* delim, struct debug_info* debug)
{
