...
static struct config_ent*
resolveUsername(struct config_ent* config, const char* uname)
{
    while (config != NULL) {
...
// requested.
static void
read_query(struct ev_loop* loop, struct client* client)
{
    char query[BUF_SIZ];
    ...
    *cr = '\0';
    ...
    if (client->query->aliasOf)
        client->query = resolveUsername(client->server->config, client->query->aliasOf);
...
static void
write_all(struct client* client)
{
...
static void
write_plan(struct client* client)
{
    size_t buf_len;
    char* buf;

    if (client->query->plan == NULL) {
        buf_len = strlen(client->query->name) + strlen(client->query->real_name) + sizeof(NO_PLAN) - 1 + sizeof(SERVER_SIG) - 1 + 30;
        buf = malloc(buf_len);
        sprintf(buf,
        ...
        buf_len = strlen(client->query->name) + strlen(client->query->real_name) + sizeof(PLAN_HEADER) - 1 + strlen(client->query->plan) + sizeof(PLAN_FOOTER) - 1 + sizeof(SERVER_SIG) - 1 + 34;
        buf = malloc(buf_len);
        sprintf(buf,
            "Username: %s\tReal Name: %s\r\n\r\n" PLAN_HEADER
...
static void
write_error(struct client* client)
{
    char* msg = NULL;
    switch (client->error) {
    case BAD_QUERY:
